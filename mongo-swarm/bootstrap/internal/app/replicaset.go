...
func (r *ReplicaSet) init() error {
	...
	if err := session.Run(bson.M{"replSetInitiate": config}, &result); err != nil {
		if err.Error() == "already initialized" {
			log.Warnf("%v replica set already initialized", r.Name)
		} else {
...
func (r *ReplicaSet) InitWithRetry(retry int, wait int) error {
	for _, member := range r.Members {
		...
		if err != nil {
			...
			log.Infof("%v member %v is online", r.Name, member)
		}
	}
...
func (r *ReplicaSet) WaitForPrimary(retry int, wait int) (bool, error) {
	...
	for retry > 0 {
		...
		retry--
		log.Warnf("No primary detected for set %v retying in %v seconds", r.Name, wait)
...
func (r *ReplicaSet) PrintStatus() error {
	...
	if err := session.Run("replSetGetStatus", &status); err != nil {
		...
		for _, m := range status.Members {
			log.Infof("%v member %v state %v", status.Name, m.Name, m.StateStr)
			if len(m.ErrMsg) > 0 {
				log.Warnf("%v member %v error %v", status.Name, m.Name, m.ErrMsg)
			}
		}
	}
